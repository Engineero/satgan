...
def _parse_example(serialized_example, a, pad_bboxes=False):
    ...
    if pad_bboxes:
        ...
        xmin = xcenter - tf.cast(10. / tf.cast(width, tf.float32), tf.float32)
        xmax = xcenter + tf.cast(10. / tf.cast(width, tf.float32), tf.float32)
        ymin = ycenter - tf.cast(10. / tf.cast(height, tf.float32), tf.float32)
        ymax = ycenter + tf.cast(10. / tf.cast(height, tf.float32), tf.float32)
    else:
